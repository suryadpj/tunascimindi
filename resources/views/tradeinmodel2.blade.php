        ...
        <div class="row py-3 px-4 px-lg-3 justify-content-center">
            @foreach ($model as $m)
                <div class="col-md-3 col-4 py-2 px-xl-3 px-lg-1 px-3 car-model-options">
                    <button id="form-1-{{ $m->model }}" onclick="OnOptionClick(this)" data-model="{{ $m->model }}" type="button" class="btn btn-primary btn-style btn-frm-2 btn-form-2"><b>{{ $m->model }}</b></button>
                </div>
            @endforeach
        </div>
    </center>
