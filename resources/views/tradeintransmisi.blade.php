...
<div id="form-4">
    ...
    <div class="row py-3 justify-content-center mx-0">
        ...
        <div class="col-md-3 col-4 py-2">
            <button id="form-4-{{ $m->transmisi }}" data-transmition="{{ $m->transmisi }}" data-variant="{{ $m->type }}" data-year="{{ $m->tahun }}" data-model="{{ $m->model }}" type="button" class="btn btn-primary btn-style btn-form-2 btn-frm-2"><b>@if($m->transmisi == 1) MT @else AT @endif</b></button>
        </div>
        @endforeach
    </div>
</div>
